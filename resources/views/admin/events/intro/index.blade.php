            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">{{trans('back.events_intro')}}</h2>
                        </div>
                    </div>
                </div>
                                        ...
                                        @foreach($intro as $row)
                                            ...
                                            <td>
                                                <div class="demo-inline-spacing">
                                                    <a href="{{ URL::route('events.edit',$row->intro_id) }}"
                                                       class="btn btn-primary"
                                                       style="position: relative;"> {{trans('back.edit')}} </a>
                                                    <a href="{{ URL::route('events.show',$row->intro_id) }}"
                                                       class="btn btn-primary"
        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-6">
                            <h2 class="content-header-title float-left mb-0">{{trans('back.events')}}</h2>
                        </div>
                        <div class="col-6">
                            <a type="button" class="btn btn-gradient-success"
                               href="{{route('events.eventCreate')}}">{{trans('back.create_events')}}</a>
                        </div>
                    </div>
                                    ...
                                    @if(isset($events))
                                        @foreach($events as $row)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="demo-inline-spacing">
                                                            <a href="{{ URL::route('events.eventEdit',$row->event_id) }}"
                                                               class="btn btn-primary"
                                                               style="position: relative;"> {{trans('back.edit')}} </a>

                                                            <a href="{{ URL::route('events.eventShow',$row->event_id) }}"
                                                               class="btn btn-warning"
                                                               style="position: relative;"> {{trans('back.show')}} </a>

                                                            <form method="POST" class="buttons-group"
                                                                  onclick="return confirm('{{trans('back.r_u_sure')}}')"
                                                                  action="{{ URL::route('events.eventDestroy',$row->event_id) }}">
                                                                {{ csrf_field() }}
                                                                {{ method_field('DELETE') }}
                                                                <button type="submit"
