                                        ...
                                        @foreach($intro as $row)
                                            ...
                                            <td>
                                                <div class="demo-inline-spacing">
                                                    <a href="{{ URL::route('partner.edit',$row->intro_id) }}"
                                                       class="btn btn-primary"
                                                       style="position: relative;"> {{trans('back.edit')}} </a>
                                                    <a href="{{ URL::route('partner.show',$row->intro_id) }}"
                                                       class="btn btn-primary"
        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-6">
                            <h2 class="content-header-title float-left mb-0">{{trans('back.partners')}}</h2>
                        </div>
                        <div class="col-6">
                            <a type="button" class="btn btn-gradient-success"
                               href="{{route('partner.item.create')}}">{{trans('back.create_partner')}}</a>
                                    ...
                                    @if(isset($partners))
                                        @foreach($partners as $row)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="demo-inline-spacing">
                                                        <a href="{{ URL::route('partner.item.edit',$row->id) }}"
                                                           class="btn btn-primary"
                                                           style="position: relative;"> {{trans('back.edit')}} </a>

                                                        <a href="{{ URL::route('partner.item.show',$row->id) }}"
                                                           class="btn btn-warning"
                                                           style="position: relative;"> {{trans('back.show')}} </a>

                                                        <form method="POST" class="buttons-group"
                                                              onclick="return confirm('{{trans('back.r_u_sure')}}')"
                                                              action="{{ URL::route('partner.item.destroy',$row->id) }}">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE') }}
                                                            <button type="submit"
